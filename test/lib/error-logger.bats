...
setup() {
  ...
  mkdir -p "${HOME}/.tmux/yoru-logs" 2>/dev/null
}

teardown() {
  cleanup_test_environment
  rm -f "${HOME}/.tmux/yoru-logs"/*.log 2>/dev/null || true
}
...
@test "error-logger.sh - log_error writes to log file when enabled" {
  ...
  [[ -f "${HOME}/.tmux/yoru-logs/errors.log" ]]
  grep -q "Test error message" "${HOME}/.tmux/yoru-logs/errors.log" || true
}

@test "error-logger.sh - log_error does not write when disabled" {
  export TMUX_ENABLE_LOGGING="0"
  rm -f "${HOME}/.tmux/yoru-logs/errors.log" 2>/dev/null || true
  log_error "test_widget" "Test error message"
  [[ ! -f "${HOME}/.tmux/yoru-logs/errors.log" ]] || true
}
...
@test "error-logger.sh - log_performance writes to log file when enabled" {
  ...
  [[ -f "${HOME}/.tmux/yoru-logs/performance.log" ]]
  grep -q "test_widget" "${HOME}/.tmux/yoru-logs/performance.log" || true
}

@test "error-logger.sh - log_performance does not write when disabled" {
  export TMUX_ENABLE_PROFILING="0"
  rm -f "${HOME}/.tmux/yoru-logs/performance.log" 2>/dev/null || true
  log_performance "test_widget" "100"
  [[ ! -f "${HOME}/.tmux/yoru-logs/performance.log" ]] || true
}

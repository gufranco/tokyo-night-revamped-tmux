...
validate_widget_enabled "@yoru_revamped_show_system"

cached_output=$(get_cached_widget_output "system")
should_use_cache "$cached_output" && echo "$cached_output" && exit 0

SHOW_CPU=$(is_widget_feature_enabled "@yoru_revamped_system_cpu" "1")
SHOW_MEMORY=$(is_widget_feature_enabled "@yoru_revamped_system_memory" "1")
SHOW_DISK=$(is_widget_feature_enabled "@yoru_revamped_system_disk" "1")
SHOW_BATTERY=$(is_widget_feature_enabled "@yoru_revamped_system_battery" "1")

WIDGET_THRESHOLD_CRITICAL=$(get_widget_threshold "@yoru_revamped_threshold_critical" "80")
WIDGET_THRESHOLD_WARNING=$(get_widget_threshold "@yoru_revamped_threshold_warning" "50")
WIDGET_WIDGET_THRESHOLD_HIGH=$(get_widget_threshold "@yoru_revamped_threshold_high" "75")
...
main() {
  ...
  if [[ $SHOW_DISK -eq 1 ]]; then
    ...
    multiple_disks=$(tmux show-option -gv @yoru_revamped_system_multiple_disks 2>/dev/null)
    ...
    if [[ $multiple_disks -eq 1 ]]; then
      ...
      disk_path=$(tmux show-option -gv @yoru_revamped_system_disk_path 2>/dev/null)
      disk_path="${disk_path:-/}"
  ...
  if [[ $SHOW_BATTERY -eq 1 ]]; then
    ...
    battery_name=$(tmux show-option -gv @yoru_revamped_system_battery_name 2>/dev/null)
    battery_threshold=$(tmux show-option -gv @yoru_revamped_system_battery_threshold 2>/dev/null)

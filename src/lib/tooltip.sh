...
generate_context_tooltip() {
  ...
  if [[ "$(tmux show-option -gv @tokyo-night-tmux_show_context 2>/dev/null)" != "1" ]]; then
    ...
    local ssh=$(tmux show-option -gv @tokyo-night-tmux_context_ssh 2>/dev/null)
    local path=$(tmux show-option -gv @tokyo-night-tmux_context_path 2>/dev/null)
    local music=$(tmux show-option -gv @tokyo-night-tmux_context_music 2>/dev/null)
    local updates=$(tmux show-option -gv @tokyo-night-tmux_context_updates 2>/dev/null)
    
    [[ "$weather" != "1" ]] && tooltip="${tooltip}Weather: Disabled\n"
    [[ "$ssh" != "1" ]] && tooltip="${tooltip}SSH: Disabled\n"
    [[ "$path" != "1" ]] && tooltip="${tooltip}Path: Disabled\n"
    [[ "$music" != "1" ]] && tooltip="${tooltip}Music: Disabled\n"
    [[ "$updates" != "1" ]] && tooltip="${tooltip}Updates: Disabled\n"
  fi

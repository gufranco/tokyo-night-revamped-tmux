  ...
  case "${os}" in
    Darwin*)
      if has_command osascript; then
        status=$(osascript -e 'tell application "System Events" to (name of processes) contains "Spotify"' 2>/dev/null)
        if [[ "$status" == "true" ]]; then
          artist=$(osascript -e 'tell application "Spotify" to artist of current track' 2>/dev/null)
          title=$(osascript -e 'tell application "Spotify" to name of current track' 2>/dev/null)
          status=$(osascript -e 'tell application "Spotify" to player state as string' 2>/dev/null)
        fi
      fi
      ;;
    Linux*)
      if has_command playerctl; then
        ...
        status=$(dbus-send --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Get string:org.mpris.MediaPlayer2.Player string:PlaybackStatus 2>/dev/null | awk '/string/ {print $3}' | tr -d '"')
        artist=$(dbus-send --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Get string:org.mpris.MediaPlayer2.Player string:Metadata 2>/dev/null | awk '/artist/ {getline; print $3}' | tr -d '"')
        title=$(dbus-send --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Get string:org.mpris.MediaPlayer2.Player string:Metadata 2>/dev/null | awk '/title/ {getline; print $3}' | tr -d '"')
      fi
      ;;
  esac
  ...
  case "${os}" in
    Darwin*)
      if has_command softwareupdate; then
        updates=$(softwareupdate -l 2>/dev/null | grep -c "Software Update found" || echo "0")
      fi
      ;;
    Linux*)
      if has_command apt; then
        ...
        updates=$(pacman -Qu 2>/dev/null | wc -l | tr -d ' ')
      elif has_command yum; then
        updates=$(yum check-update --quiet 2>/dev/null | grep -c "^[a-zA-Z]" || echo "0")
      fi
      ;;
  esac
...
get_bluetooth_status() {
  ...
  case "${os}" in
    Darwin*)
      if has_command blueutil; then
        status=$(blueutil --power 2>/dev/null)
        devices=$(blueutil --paired 2>/dev/null | grep -c "connected" || echo "0")
      elif has_command system_profiler; then
        status=$(system_profiler SPBluetoothDataType 2>/dev/null | grep -q "Bluetooth: On" && echo "1" || echo "0")
      fi
      ;;
    Linux*)
      if has_command bluetoothctl; then
        status=$(bluetoothctl show 2>/dev/null | grep -q "Powered: yes" && echo "1" || echo "0")
        devices=$(bluetoothctl devices Connected 2>/dev/null | wc -l | tr -d ' ')
      elif [[ -f /sys/class/bluetooth ]]; then
        status=$(ls /sys/class/bluetooth 2>/dev/null | wc -l)
        [[ $status -gt 0 ]] && status="1" || status="0"
      fi
      ;;
  esac
...
get_audio_device() {
  ...
  case "${os}" in
    Darwin*)
      if has_command SwitchAudioSource; then
        device=$(SwitchAudioSource -c 2>/dev/null)
      elif has_command osascript; then
        device=$(osascript -e 'tell application "System Events" to get name of current audio device' 2>/dev/null)
      fi
      ;;
    Linux*)
      if has_command pactl; then
        device=$(pactl get-default-sink 2>/dev/null | xargs -I {} pactl list sinks short | awk '/{}/ {print $2; exit}')
      elif has_command amixer; then
        device=$(amixer scontrols 2>/dev/null | head -1 | awk -F"'" '{print $2}')
      fi
      ;;
  esac
...
get_screen_brightness() {
  ...
  case "${os}" in
    Darwin*)
      if has_command brightness; then
        brightness=$(brightness -l 2>/dev/null | awk '/display 0: brightness/ {print $4 * 100; exit}')
      fi
      ;;
    Linux*)
      if [[ -d /sys/class/backlight ]]; then
        ...
        for backlight in /sys/class/backlight/*; do
          if [[ -f "${backlight}/max_brightness" ]] && [[ -f "${backlight}/brightness" ]]; then
            ...
            current_brightness=$(cat "${backlight}/brightness" 2>/dev/null)
            if [[ -n "$max_brightness" ]] && [[ -n "$current_brightness" ]] && [[ $max_brightness -gt 0 ]]; then
              brightness=$(( (current_brightness * 100) / max_brightness ))
              break
            fi
          fi
        done
      elif has_command xbacklight; then
        brightness=$(xbacklight -get 2>/dev/null | awk '{print int($1)}')
      fi
      ;;
  esac

  [[ -n "$brightness" ]] && [[ "$brightness" =~ ^[0-9]+$ ]] && echo "$brightness" || echo "0"
}
...
export -f get_music_player_status
export -f get_system_updates
export -f get_bluetooth_status
export -f get_audio_device
export -f get_screen_brightness

...
generate_system_tooltip() {
  ...
  if [[ "$(tmux show-option -gv @yoru_show_system 2>/dev/null)" != "1" ]]; then
    tooltip="${tooltip}System widget: Disabled\n"
  else
    local cpu=$(tmux show-option -gv @yoru_system_cpu 2>/dev/null)
    local mem=$(tmux show-option -gv @yoru_system_memory 2>/dev/null)
    local disk=$(tmux show-option -gv @yoru_system_disk 2>/dev/null)
    local battery=$(tmux show-option -gv @yoru_system_battery 2>/dev/null)
...
generate_git_tooltip() {
  ...
  if [[ "$(tmux show-option -gv @yoru_show_git 2>/dev/null)" != "1" ]]; then
    tooltip="${tooltip}Git widget: Disabled\n"
  else
    local web=$(tmux show-option -gv @yoru_git_web 2>/dev/null)
    local stash=$(tmux show-option -gv @yoru_git_stash 2>/dev/null)
    local ahead=$(tmux show-option -gv @yoru_git_ahead_behind 2>/dev/null)
...
generate_network_tooltip() {
  ...
  if [[ "$(tmux show-option -gv @yoru_show_netspeed 2>/dev/null)" != "1" ]]; then
    tooltip="${tooltip}Network widget: Disabled\n"
  else
    local ping=$(tmux show-option -gv @yoru_netspeed_ping 2>/dev/null)
    local vpn=$(tmux show-option -gv @yoru_netspeed_vpn 2>/dev/null)
    local wifi=$(tmux show-option -gv @yoru_netspeed_wifi 2>/dev/null)
...
generate_context_tooltip() {
  ...
  if [[ "$(tmux show-option -gv @yoru_show_context 2>/dev/null)" != "1" ]]; then
    tooltip="${tooltip}Context widget: Disabled\n"
  else
    local weather=$(tmux show-option -gv @yoru_context_weather 2>/dev/null)
